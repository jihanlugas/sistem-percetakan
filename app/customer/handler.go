...
// @Accept json
// @Produce json
// @Param req query request.PageCustomer false "url query string"
...
func (h Handler) GetById(c echo.Context) error {
	...
	vCustomer, err := h.usecase.GetById(loginUser, id, preloadSlice...)
	if err != nil {
		return response.Error(http.StatusBadRequest, err.Error(), err, nil).SendJSON(c)
	}

	return response.Success(http.StatusOK, response.SuccessHandler, vCustomer).SendJSON(c)
}
...
// @Accept json
// @Produce json
// @Param req body request.CreateCustomer true "json req body"
// @Success      200  {object}	response.Response
// @Failure      500  {object}  response.Response
...
func (h Handler) Create(c echo.Context) error {
	...
	req := new(request.CreateCustomer)
	if err = c.Bind(req); err != nil {
		return response.Error(http.StatusBadRequest, response.ErrorHandlerBind, err, nil).SendJSON(c)
	}
...
// @Produce json
// @Param id path string true "ID"
// @Param req body request.UpdateCustomer true "json req body"
// @Success      200  {object}	response.Response
// @Failure      500  {object}  response.Response
...
func (h Handler) Update(c echo.Context) error {
	...
	req := new(request.UpdateCustomer)
	if err = c.Bind(req); err != nil {
		return response.Error(http.StatusBadRequest, response.ErrorHandlerBind, err, nil).SendJSON(c)
	}
